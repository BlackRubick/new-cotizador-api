...
router.post('/', requireAuth, async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    console.log('encargados:', body.encargados);
    ...
    // Preparar metadata con encargados
    const metadata = body.metadata || {};
    if (body.encargados) {
      metadata.encargados = body.encargados;
    }
...
router.put('/:id', requireAuth, async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    console.log("=== PUT /clients/:id ===");
    console.log("Body recibido:", JSON.stringify(body, null, 2));
    console.log("encargados en body:", body.encargados);

    // Preparar datos para actualizar
    const updateData: any = {};
    if (body.name) updateData.name = body.name;
    if (body.nombre) updateData.name = body.nombre;
    ...
    // Manejar metadata con encargados
    if (body.metadata !== undefined || body.encargados !== undefined) {
      const currentClient = await prisma.client.findUnique({ where: { id } });
      const currentMetadata = (currentClient?.metadata as any) || {};
      
      updateData.metadata = {
        ...currentMetadata,
        ...(body.metadata || {}),
        encargados: body.encargados || currentMetadata.encargados || []
      };
    }
